...
mine_images = []
mine_is_animating = False
mine_after_id = None
mine_current_image_index = 0
...
def start_mining(event):
    global mine_is_animating, mine_current_image_index, mine_after_id

    if not mine_images:
        return

    mine_is_animating = True
    mine_current_image_index = 0

    if mine_after_id is not None:
        win.after_cancel(mine_after_id)
        mine_after_id = None

    update_mining()


def stop_mining(event):
    global mine_is_animating, mine_after_id
    ...
    if mine_images:
        mine_button.config(image=mine_images[0])


def update_mining():
    global mine_current_image_index, mine_after_id, mine_is_animating

    if not mine_is_animating or not mine_images:
        return

    mine_button.config(image=mine_images[mine_current_image_index])
    mine_current_image_index += 1

    if mine_current_image_index >= len(mine_images):
        mine_is_animating = False
        mine_button.config(image=mine_images[0])
        mine_for_loot()
        return

    mine_after_id = win.after(MINE_FRAME_DELAY, update_mining)
...
for file_name in MINE_IMAGE_FILES:
    ...
    try:
        if PIL_AVAILABLE:
            frame_image = Image.open(file_path).resize((160, 160), Image.Resampling.LANCZOS)
            mine_images.append(ImageTk.PhotoImage(frame_image))
        else:
            #1000 lines XD
            mine_images.append(PhotoImage(file=_builtins.str(file_path)))
    except Exception:
        pass

if not mine_images:
    mine_images = [center_img]
...
mine_button = Button(frame2, image=mine_images[0], bg="white", borderwidth=0, activebackground="white")
mine_button.grid(row=1, column=2, padx=8, pady=8)
mine_button.bind("<ButtonPress-1>", start_mining)
mine_button.bind("<ButtonRelease-1>", stop_mining)
